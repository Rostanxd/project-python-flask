...
def test_add_users(client, create_test_role, active_user, inactive_user):
...
def test_remove_users(client, role_with_users, active_user, inactive_user):
...
def test_invalid_user_payload(client, create_test_role, active_user):
    # Call API removing all users from this role
    response = client.post(
...
def test_role_does_not_exist(client):
    # Send a POST request to update role's users
    response = client.post(
...
def test_invalid_user_id(client, create_test_role):
    # Send a POST request to update role's users
    response = client.post(
        "/roles/{}/users".format(create_test_role.role_id),
        json={"user_ids": [ACTIVE_USER.get("id")]},
